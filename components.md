...
- [ ] BaseStyles (focus-visible) (partially implemented in `src/components/BaseStyles`; includes upstream `as` support, `color` CSS variable handling, `data-portal-root`, and stable exports/tests, but still lacks full upstream focus-visible polyfill parity)
- [x] Blankslate (Button, Link) (implemented in `src/components/Blankslate`; includes `Visual`, `Heading`, `Description`, `PrimaryAction`, `SecondaryAction`, sizing context, and CSS module styling semantics)
- [x] BranchName (none)
...
- [x] ButtonGroup (implemented in `src/components/ButtonGroup`; includes upstream wrapper structure, CSS module styling semantics, polymorphic `as`, and toolbar horizontal arrow-key focus wrap)
- [x] Card (none)
...
- [x] CircleBadge (Octicon)
- [ ] ConfirmationDialog (BaseStyles, Dialog, react-dom/client) (not implemented)
- [x] CounterLabel (VisuallyHidden)
- [x] DataTable (Table) (implemented in `src/components/DataTable`; includes column helpers, table rendering, row headers, initial/client/external sorting, skeleton state, and core Table composition API)
...
- [ ] PageLayout (useId, useOverflow, useRefObjectAsForwardedRef, useResponsiveValue, useSlots) (partially implemented in `src/components/PageLayout`; includes `Header`, `Content`, `Pane`, `Sidebar`, `Footer`, root slot composition, responsive divider/position attrs, and resizable pane/sidebar styling/interaction, but still needs broader upstream parity validation)
- [ ] Pagehead (none) (not implemented)
- [x] Pagination (@primer/octicons-react, useResponsiveValue) (implemented in `src/components/Pagination`; includes upstream page model, prev/next controls, truncation, `renderPage`, data-component attrs, and responsive page hiding via `showPages`)
...
- [x] ProgressBar (none)
- [x] Radio (Checkbox, RadioGroup)
- [x] RadioGroup (hooks)
- [x] RelativeTime (@github/relative-time-element) (implemented in `src/components/RelativeTime`; includes SSR fallback text, `date`/`datetime` parity, `noTitle`, and custom event callback wiring)
- [ ] ScrollableRegion (useOverflow) (not implemented)
- [ ] SegmentedControl (ActionList, ActionMenu, react-is, useResponsiveValue) (partially implemented in `src/components/SegmentedControl`; includes `SegmentedControl.Button`, `SegmentedControl.IconButton`, controlled/uncontrolled selection, responsive `fullWidth`/`variant` data attrs, and a basic dropdown fallback/tooltip implementation, but still needs broader upstream ActionMenu/ActionList integration parity validation)
...
- [x] Table (@primer/octicons-react, DataTable, ScrollableRegion, SkeletonText, Text, VisuallyHidden) (implemented in `src/components/DataTable`; includes `Table.Container`, `Title`, `Subtitle`, `Actions`, `Divider`, `Skeleton`, `Head`, `Body`, `Header`, `Row`, `Cell`, and `CellPlaceholder`, with sortable-header support used by `DataTable`)
...
- [ ] ThemeProvider (hooks, useSyncedState) (partially implemented in `src/components/ThemeProvider`; includes day/night schemes, auto/system mode, inherited nested schemes, setters, top-level `theme` export integration, and `useColorSchemeVar`, but still lacks upstream SSR handoff/prevent-mismatch behavior and deep theme merge parity)
- [x] Timeline (none)
